...
public abstract class MultiRoom extends PseudoRoom {
	...
	public MultiRoom(final ConfigurationSection config) {
		...
		for (final String key : roomConfig.getKeys(false)) {
			final Room room = PseudoRoom.load(roomConfig.getConfigurationSection(key));
			if (room == null) {
				throw new IllegalArgumentException("Invalid Room Configuration in \"" + roomConfig.getCurrentPath() + "." + key + "\" !");
			}
			rooms.add(room);
		}
	}
